...
inline $ZTYPE[*] $FNAMEQUADav($XTYPE[256] x, $YTYPE y$SYSVARGDECL)
...
% -------------------- epsilon fragments -----------------------------
%
%Fragment eps 000 bic bic b .
inline $ZTYPE $FNAME($XTYPE x, $YTYPE y)
{ /* Non-fuzzy scalar member scalar */
 z = to$CT(x) == to$CT(y);
 return(z);
}
...
inline $ZTYPE $FNAME($XTYPE x, $YTYPE[.] y$SYSVARGDECL)
{ /* Non-fuzzy S member V */
 sy = (shape(y))[[0]];
 z = false;
 for(i=0; i<sy; i++) {
        if (to$CT(x) == to$CT(y[[i]])){
                z = true;
                i = sy;
        }
 }
...
{ /* Fuzzy S member V */
 ...
 z = false;
 for(i=0; i<sy; i++) {
        if (eqDDB(to$CT(x),to$CT(y[[i]]),QUADct)){
                z = true;
                i = sy;
        }
 }
...
inline $ZTYPE[*] $FNAME($XTYPE[+] x, $YTYPE y$SYSVARGDECL)
{ /* Non-fuzzy A member S */
 z = with(. <= iv <= .) 
	genarray(shape(x), 
	(to$CT(x),to$CT(y),false);
 return(z);
}
...
{ /* Chars member chars */
 ...
 z = with (. <= iv <= .)
    genarray(shape(x), tbl[[toi(x[iv])]], false);
 return(z);
}
 
%Fragment eps *** c c b QUADav
inline $ZTYPE[*] $FNAMEQUADav($XTYPE[*] x, $YTYPE[256] y$SYSVARGDECL)
...
inline $ZTYPE[*] $FNAME($XTYPE[+] x, $YTYPE[+] y$SYSVARGDECL)
{ /* A member A */
 ravely = to$CT(comaX$YT$YT(y));
 gotit = (shape(ravely))[[0]];
 z = with (. <= iv <= .)
	genarray(shape(x), 
		/* DUMB search */
		 gotit != iota$XT$YTI(ravely,x[iv]$SYSVARGKER,0),false);
 return(z);
}
%Generate , coma, X$YT$YT, X*1, ., $YT
%Generate , iota, $XT$YTI, 100, ., $CT

%Fragment junk 111 bidc bidc bidc .
/* what is this stuff? */
...
$SFCASES{{ % General case
 z = with(. <= iv <= .) {
	genarray(shape(x), 
		((shape(y))[[0]] != FindFirst(y,x,QUADct,z,i)));
} % End of general case
{ % Special case for character epsilon quadav
 reshape(shape(x),true);
}
