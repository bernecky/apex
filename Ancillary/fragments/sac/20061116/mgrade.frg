...
%Fragment ugrdPV x11 x bi i PV   Vector upgrade of permutation vector
inline $ZTYPE[.] $FNAME($YTYPE[.] y, int QUADio)
...
%Fragment dgrdPV x11 x bi i  PV downgrade on permutation vector
inline $ZTYPE[.] $FNAME($YTYPE[.] y, int QUADio)
...
{ /* Boolean Array Comparator for upgrade */
 ...
 z = with{
       (0*shape(x) <= iv < shape(x)): x[iv] > y[iv];
        }: foldfix( &, true,false);
 return(z);
}
