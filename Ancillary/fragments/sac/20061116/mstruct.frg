...
{ /* Tranpose and rotate on scalars are NOPs */ 
return(y);
}
...
inline $ZTYPE[.,1] $FNAME($YTYPE[.] y)
{ /* Table on vector */
 z = reshape([shape(y),1],y);
 return(z);
}
...
{ /* Table on matrix */
 shpy = shape(y);
 z = reshape(take([1],shpy),prod(drop([1],shpy)),y);
 return(z);
}
...
{ /* First axis reverse on anything */
 ...
 z = with( . <= iv <= .)
        genarray(frameshape, y[(frameshape-[1])-iv],cell);
 return(z);
}
...
{/* Last axis reverse on rank>1 */
 ...
 z = with( . <= iv <= .)
	genarray(frameshape, $FNAME(y[iv]),cell);
 return(z);
}
%Generate , $FN, $XT$YT$ZT, x11, ., $CT
