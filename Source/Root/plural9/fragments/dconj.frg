...
$ZTYPE $FNAME$IPALG($XTYPE x, $YTYPE y$SYSVARFGDECL)
...
%Generate , $FRVALUE, $CT$CT$GZT, 111, ., $CT
%Generate $FNM,	sl,	 X$GZT$ZT,   X10, $SPECRED, $GZT
...
{ /* TRANSPOSE case of inner product z = scalar f.g y */
 ...
 z = with {
	(. <= iv <= .) {
		vy = yt[iv];
	 } : $FNMslX$GZT$ZT$SPECRED($FRVALUE$GCT$GCT$GZT(xct,vy$SYSVARGKER)$SYSVARFKER);
	} : genarray(shp, $OTFILL);
 return(z);
}
...
{ /* Inner product z =  f.g scalar-y */
 ...
 z = with {
	(. <= iv <= .) {
	} : $FNMslX$GZT$ZT$SPECRED($FRVALUE$GCT$GCT$GZT(to$GCT(x[iv]),to$GCT(y)$SYSVARGKER)$SYSVARFKER);
	} : genarray(shp, $OTFILL);
 return(z);
}
%Generate , 	$FRVALUE, $GCT$GCT$GZT, 101, ., $GCT
%Generate $FNM,	sl,	 X$GZT$ZT,   X10, $SPECRED, $GZT
...
{ /* TRANSPOSE case of inner product z = vector_f.g y */
 ...
 z = with {
	...
	} : $FNMslX$GZT$ZT$SPECRED($FRVALUE$GCT$GCT$GZT(vx,vy$SYSVARGKER)$SYSVARFKER);
	} :genarray(shp, $OTFILL);
 return(z);
}
%Generate , 	TRANSPOSE, X$YT$YT, X**, ., $YT
%Generate , 	$FRVALUE, $GCT$GCT$GZT, 111, ., $GCT
%Generate $FNM, sl, X$GZT$ZT,   X10, $SPECRED, $GZT

%Fragment dot *1* bidc bidc bidc STAR
%Fragment dot *2* bidc bidc bidc STAR
$ZTYPE $FNAME$IPALG($XTYPEx, $YTYPE y$SYSVARFGDECL)
  ...
  */
  ...
  z = with {
    (. <= [row] <= .) {
      ...
      for (colx=0; colx<colsx; colx++) {
        xel = to$GCT(xrow[colx]);
        if (to$GCT($FGID) != xel) { /* Skip iteration if it's an f identity */
          Crow = $FNM$ZT$ZT$ZT( to$CT( y[colx]), Crow $SYSVARFKER);
        }
      }
    } : Crow;
  }: genarray( rowsx, Zrow);
...
{ /* QUICKSTOP0 case of inner product z = matrix f.g matrix */
 ...
 z = with {
   (. <= iv <= .) {
      vx = xct[take([dim(x)-1], iv)];
      vy = yt[ reverse(take([1-dim(y)], iv))];
    } : $FNMslX$GZT$ZTQUICKSTOP($FRVALUE$GCT$GCT$GZT(vx,vy$SYSVARGKER)$SYSVARFKER);
 } : genarray(shp, $OTFILL);
 return(z);
}
%Generate , 	TRANSPOSE, X$YT$YT, X**, ., $YT
