 r←EmitMonadicAdverbs fns;cv;sis;ctl;prim;fn;f;fns2;cvm;rnk;tp;sc;sov;lid
 ...
 :If ~0∊⍴fns
   ...
   ⍝ Have to do these now or they get stomped
         sis←(E'$FNAME')Strepl¨ER1(⍪sis),Raze⍉f[;fnslop,fnsfn,fnstypes]
         sis←(E'$FN')Strepl¨ER1(⍪sis),Raze⍉f[;,fnslop]
         f←TSAC(D f[;fnstypes])[;,fnstypesrarg]
         sis←(E'$YTYPE')Strepl¨ER1(⍪sis),f
         f←fnslop ConjToFns cv⌿fns       ⍝ Set f/ left identity
         lid←EmitReduceFixLid f
         sis←(E'$FRID')Strepl¨ER1(⍪sis),lid
         sov←EmitReduceStopOnValue cv⌿fns
         sis←(E'$STOPONVALUE')Strepl¨ER1(⍪sis),sov
