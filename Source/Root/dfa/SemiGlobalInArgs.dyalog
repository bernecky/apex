 r←cds SemiGlobalInArgs ast;nast;tar;astr;j;wl
⍝ Propagate Semiglobal dfa info from ast of caller
⍝ to the callee
 nast←D cds[ssaast] ⍝ Callee's ast
 Checkem(E'?'),E nast
 wl←D cds[ssacv] ⍝ Worklist
 tar←UnSSAName¨ast[;asttarget] ⍝ Get original name again
 tar←D(UnSSAName¨nast[;asttarget])⍳tar ⍝ Find the targets
 astr←nast[tar;]                ⍝ Affected rows in subfn
 astr[;astchar]←ast[;astchar]   ⍝ Update subfn's ast
